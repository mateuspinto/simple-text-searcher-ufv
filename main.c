...
int main(void) {
  ...
  while (option != 0) {
    if (option == 1) {
      ...
      printf("Para usar o nosso software de pesquisa selecione uma das "
             "opções:\n\n");
      ...
      while (optionSearch != 0) {
        ...
        generalFunctionsShowRadicalsAutoFill(&tstAutoFill, &listAutoComplete,
                                             searchTemp);

        printf("AutoComplete - Digite o indice da palavra que vc deseja "
               "pesquisar: ");

        scanf("%d", &index);

        generalFunctionsGetRadical(&listAutoComplete, searchTemp, search,
                                   index);

        printf("Para inserir mais palavras digite '1', para pesquisar digite "
               "'0': ");
      ...
      printf("Ops! A opção que vc selecionou não existe, por favor selecione "
             "uma opção válida ou digite '1' para retornar ao menu: ");
      scanf("%d", &option);
      setbuf(stdin, NULL);
    }
  }
