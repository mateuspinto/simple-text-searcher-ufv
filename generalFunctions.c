...
int generalFunctionsSetNumDifferentsWords(tstNode **arvoreAuxiliar,
                                          tstFileNode **tstFile, char *filename,
                                          char *word) {
...
int generalFunctionsLoadWords(FILE *fp, char *filename, tstNode **tstAutoFill,
                              patriciaNode **patricia, tstFileNode **tstFile) {
  char word[45];
  char c;
  int count;
  ...
  while ((c = fgetc(fp)) != EOF) {
    if ((c == ' ' || c == '\n') && count != 0) {
      ...
      generalFunctionsSetNumDifferentsWords(&arvoreAuxiliar, tstFile, filename,
                                            word);
      count = 0;
    } else {
      if ((c >= 97 && c <= 122) || // Verifica se e uma letra minuscula
        ...
        if (c >= 65 &&
            c <= 90) // Caso seja uma letra maiscula, transforma numa minuscula
          c = c + 32;
...
int generalFunctionsAuxLoadTstFile(tstFileNode *tstFile, char *buffer, int h,
                                   tstNode **tstAutoFill,
...
int generalFunctionsSearch(tstFileNode **tstFile, patriciaNode **patricia,
                           char *words, int qtdFiles) {
  ...
  // Reparte a String pelos seus espaço e pesquisa palavra por palavra
  // atribuindo os arquivos que ela aparece em uma lista encadeada tempoária
  while (word) {
    invertedChainedList *listTemp = patriciaNodeSearchWord(patricia, word);
    int qtdFilesAsWord = 0;
    // Função recursiva auxiliar para descobrir e atribuir o nome dos arquivos e
    // os seus respectivos pesos na lista encadeada
    generalFunctionsSearchWordAux(tstFile, &listTemp, &listaPesquisaTemp,
                                  qtdFiles, &qtdFilesAsWord);
    // puts(word);
    word = strtok(NULL, " ");
    x++;
  }
  ...
  if (listaPesquisaTemp != NULL) {
    ...
    // Carrega a lista na BST com obejtivo de ordenar os resultados em relação
    // ao seu peso
...
// Essa função navega até o fim da lista encadeada invertida e só no seu retorno
// faz os calculos dos pesos, ou seja, ela funciona do final para o inicio da
// lista
int generalFunctionsSearchWordAux(tstFileNode **tstFile,
                                  invertedChainedList **listFiles,
                                  listaPesquisa **listaTemp, int qtdFiles,
                                  int *qtdFilesAsWord) {
  if (*listFiles != NULL) {
    (*qtdFilesAsWord) += 1;
    int ocurrences = (**listFiles).ocurrences;
    int numDifferentsWords =
        (**tstFileNodeSearch(tstFile, (**listFiles).filename))
            .numDifferentsWords;
    char *filename = (**listFiles).filename;
    double weight = 2.0;
    generalFunctionsSearchWordAux(tstFile, &(**listFiles).next, listaTemp,
                                  qtdFiles, qtdFilesAsWord);

    weight = (1.0 / ((double)numDifferentsWords)) * (double)ocurrences *
             (log2((double)qtdFiles) / (double)(*qtdFilesAsWord));
...
int generalFunctionsLoadListInBST(listaPesquisa **listaPesquisaTemp,
                                  bstNode **bstPesquisa) {
  if (*listaPesquisaTemp != NULL) {
    bstNodeInsertFile(bstPesquisa, (**listaPesquisaTemp).wieght,
                      (**listaPesquisaTemp).filename);
...
int generalFunctionsShowRadicalsAutoFill(tstNode **node, listAutoFill **list,
                                         char *word) {
...
int generalFunctionsGetRadical(listAutoFill **list, char *word, char *string,
                               int index) {
