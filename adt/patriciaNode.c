...
int patriciaNodeCheckBitFlow(patriciaNode **node, char *word) {
  ...
  if (word[(**node).node.internal.position] >=
      (**node).node.internal.character) {
    return 1;
  }
...
int patriciaNodeCreateExternalNode(patriciaNode **node, char *word,
                                   char *filename) {
  int wordSize = 1;
...
int patriciaNodeCreateInternalNode(patriciaNode **node, char character,
                                   int position, patriciaNode **left,
                                   patriciaNode **right) {
  (*node) = malloc(sizeof(patriciaNode));
...
int patriciaNodeWhichIsDifferent(patriciaNode **node, char *word) {
  ...
  while ((((**node).node.external.word)[position]) == (word[position])) {
    ...
    if ((((**node).node.external.word)[position]) == '\0' &&
        (word[position]) == '\0') {
      return -1;
...
int patriciaNodeInsertBetween(patriciaNode **node, char *word, int position,
                              char character, char *filename) {
  ...
  if (patriciaNodeIsExternal(node)) {
    ...
    if (word[position] > internalNodeChar) {
      return patriciaNodeCreateInternalNode(node, word[position], position,
                                            &swap, &createdNode);
    } else {
      return patriciaNodeCreateInternalNode(node, internalNodeChar, position,
                                            &createdNode, &swap);
    }
  }
  ...
  if (position < (**node).node.internal.position) {
    ...
    if (word[position] >= character) {
      return patriciaNodeCreateInternalNode(node, character, position, &swap,
                                            &createdNode);
    } else {
      return patriciaNodeCreateInternalNode(node, character, position,
                                            &createdNode, &swap);
    }
  }

  if (position == (**node).node.internal.position &&
      word[position] > patriciaNodeReturnCharacter(
                           node, position)) { // MENOR OU MENOR E IGUAL??
  ...
  if (patriciaNodeCheckBitFlow(node, word)) {
    return patriciaNodeInsertBetween(&((**node).node.internal.right), word,
                                     position, character, filename);
  } else {
    return patriciaNodeInsertBetween(&((**node).node.internal.left), word,
                                     position, character, filename);
  }
...
int patriciaNodeIncrementOcurrence(patriciaNode **node, char *filename) {
  ...
  return invertedChainedListInsertNode(&((**node).node.external.textList),
                                       filename);
}
...
int patriciaNodeInsertWord(patriciaNode **node, char *word, char *filename) {
  ...
  return patriciaNodeInsertBetween(node, word, differPosition, character,
                                   filename);
}
...
invertedChainedList *patriciaNodeSearchWord(patriciaNode **node, char *word) {
  if (*node != NULL) {
    if ((**node).nodeType == internal) {
      if ((**node).node.internal.character <=
          word[(**node).node.internal.position]) {
